...
Example:
```yaml
- descriptor: "pip-services-paymentmethods:persistence:memory:default:1.0"
  options:
    max_page_size: 100
```
...
- path: string - file path where PaymentMethodV1 objects are stored. The object are written into the file in JSON format.
- options: object - Misc configuration options
  - max_page_size: number - Maximum number of items per page (default: 100)
